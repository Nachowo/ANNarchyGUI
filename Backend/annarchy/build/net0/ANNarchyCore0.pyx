...
cdef extern from "ANNarchy.h":
    ...
    # Export Population 0 (LIF_Neuron1)
    ...
    # Export Population 1 (LIF_Neuron2)
    ...
    # Population 0 (LIF_Neuron1) : Monitor
    ...
    # Population 1 (LIF_Neuron2) : Monitor
...
# Wrapper for population 0 (LIF_Neuron1)
...
# Wrapper for population 1 (LIF_Neuron2)
@cython.auto_pickle(True)
...
cdef class ProjRecorder0_wrapper:
    ...
    property I:
        def __get__(self): return (ProjRecorder0.get_instance(self.id)).I
        def __set__(self, val): (ProjRecorder0.get_instance(self.id)).I = val
    property record_I:
        def __get__(self): return (ProjRecorder0.get_instance(self.id)).record_I
        def __set__(self, val): (ProjRecorder0.get_instance(self.id)).record_I = val
    def clear_I(self):
        (ProjRecorder0.get_instance(self.id)).I.clear()
